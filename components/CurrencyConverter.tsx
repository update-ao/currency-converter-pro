...
const timeRangePresets: TimeRangePreset[] = [
 { labelKey: 'timeRange7D' as TranslationKey, value: '7D', days: 7 },
 { labelKey: 'timeRange1M' as TranslationKey, value: '1M', days: 30 },
 { labelKey: 'timeRange1Y' as TranslationKey, value: '1Y', months: 12 },
];
...
const regionOrder: RegionName[] = ['Africa', 'Asia', 'Europe', 'NorthAmerica', 'Oceania', 'SouthAmerica'];
...
const CurrencyConverter: React.FC = () => {
 ...
 const [toCurrency, setToCurrency] = useState<string>('aoa'); // Default to Angolan Kwanza (AOA)
 ...
 const getKnownSymbol = useCallback((currencyCode: string): string | undefined => {
  ...
  const getCurrencySymbol = useCallback((currencyCode: string, locale: string): string => {
   ...
   try {
     // Attempt to get narrow symbol first
     const partsNarrow = Intl.NumberFormat(locale, { style: 'currency', currency: currencyCode.toUpperCase(), currencyDisplay: 'narrowSymbol' }).formatToParts(0);
     const symbolPartNarrow = partsNarrow.find(part => part.type === 'currency');
     if (symbolPartNarrow && symbolPartNarrow.value !== currencyCode.toUpperCase()) return symbolPartNarrow.value;

    // Fallback to standard symbol if narrow symbol is just the currency code
     ...
     const symbolPartStandard = partsSymbol.find(part => part.type === 'currency');
     if (symbolPartStandard && symbolPartStandard.value !== currencyCode.toUpperCase()) return symbolPartStandard.value;
    
   } catch (e) {
       console.warn(`Could not get symbol for ${currencyCode} via Intl.NumberFormat:`, e);
   }
   // Final fallback to the currency code itself if Intl fails or doesn't provide a distinct symbol
 ...
 const formatCurrencyValue = useCallback((value: number, locale: string, minFractionDigits: number = 2, maxFractionDigits: number = 2): string => {
   try {
     ...
     // Replace standard group separator (like comma in en-US) with a space for better readability if needed,
     // or customize further based on specific locale requirements.
     // For now, using default locale formatting and then replacing group to space.
  ...
  const getRelativeTimeString = useCallback((apiDateString: string, currentLocale: string): string => {
   const apiDate = new Date(apiDateString + 'T00:00:00Z'); // Assume API date is UTC midnight
   const now = new Date();
  
   // Calculate difference in seconds, then days, considering only the date part
   const diffSeconds = Math.round((now.getTime() - apiDate.getTime()) / 1000);
   const diffDays = Math.floor(diffSeconds / (3600 * 24));

    // Get today's date at UTC midnight for accurate comparison
   ...
   if (apiDateUtcMidnight.getTime() === todayUtc.getTime()) {
       // If the API date is today (UTC), show hours ago or "just now"
       const hoursSinceMidnightUtc = now.getUTCHours(); // Current UTC hour
       if (hoursSinceMidnightUtc < 1) return t('updatedNow'); // Updated within the last hour
       return t('updatedHoursAgo', hoursSinceMidnightUtc.toString());
   }
   
   // Check for yesterday
   // Create a date object for yesterday at UTC midnight
   const yesterdayUtc = new Date(todayUtc);
   yesterdayUtc.setUTCDate(todayUtc.getUTCDate() - 1);

   if (apiDateUtcMidnight.getTime() === yesterdayUtc.getTime()) {
       return t('updatedYesterday');
   }
   
   // For other dates, show the formatted date
   // Ensure the date is formatted according to the locale, specifying UTC to avoid timezone shifts in display
   ...
   const fetchCurrencies = async () => {
     ...
     try {
       ...
       if (availableCurrencyCodes.length > 0) {
           // Ensure fromCurrency is valid, default if not
           if (!availableCurrencyCodes.includes(fromCurrency.toLowerCase())) {
               currentFrom = availableCurrencyCodes[0]; // Default to the first available
               setFromCurrency(currentFrom);
           }
           // Ensure toCurrency is valid and different from fromCurrency, default if not
           if (!availableCurrencyCodes.includes(toCurrency.toLowerCase()) || toCurrency.toLowerCase() === currentFrom.toLowerCase()) {
               currentTo = availableCurrencyCodes.find(k => k.toLowerCase() !== currentFrom.toLowerCase()) || (availableCurrencyCodes.length > 1 ? availableCurrencyCodes[1] : availableCurrencyCodes[0]);
               setToCurrency(currentTo);
           }
       } else {
           // Fallback if no currencies are available after filtering (should not happen with a good list)
           setFromCurrency('usd'); // Default
           setToCurrency('eur'); // Default
       }
       ...
       const sortedRegionOrder = [...regionOrder].sort((a, b) => {
           ...
           return nameA.localeCompare(nameB, language); // Use current language for sorting
       });
       ...
       Object.entries(filteredCurrencies)
         .forEach(([code, name]) => {
           ...
           if (region) { // Only include currencies that have a defined region
               const flagEmoji = getFlagEmojiForCurrency(code);
               const label = `${flagEmoji} ${name} (${code.toUpperCase()})`.trim();
               grouped[region].push({
                   value: code, // Store original case for API calls if needed, though service might handle lowercase
                   label: label,
               });
           }
         });
       ...
       // Create grouped options for the selector, maintaining the sorted region order
       const groupedOptions: GroupedCurrencyOption[] = sortedRegionOrder // Use sortedRegionOrder
           .map(regionName => {
               ...
               if (currencies && currencies.length > 0) {
                   // Sort currencies alphabetically within the region by their label
                   currencies.sort((a, b) => a.label.localeCompare(b.label, language));
                   return {
                       regionKey: regionTranslationKeys[regionName], // Use the translation key for the region label
                       currencies: currencies,
                   };
               }
               return null; // In case a region has no currencies after filtering (shouldn't happen with good data)
           })
           .filter(group => group !== null) as GroupedCurrencyOption[]; // Filter out any null groups
       ...
       setCurrencyOptions([]); // Reset options on error
       setAllCurrenciesMap({}); // Reset map on error
     } finally {
 ...
 }, [t, language, fromCurrency, toCurrency]); // Added fromCurrency and toCurrency to re-validate them if language changes or initial load


 const fetchLatestRates = useCallback(async () => {
   if (!fromCurrency) return; // Don't fetch if fromCurrency isn't set


   setIsLoadingRates(true);
   setError(null);
   setConvertedAmount(null); // Reset previous conversion
   try {
     ...
     setExchangeRateData(null); // Clear old data on error
   } finally {
 ...
 // Effect to fetch rates when fromCurrency changes or when currency options are loaded
 useEffect(() => {
   if (fromCurrency && currencyOptions.length > 0) { // Ensure options are loaded before fetching
       fetchLatestRates();
   }
 }, [fetchLatestRates, fromCurrency, currencyOptions.length]); // Rerun if fromCurrency or options change


 // Effect to calculate converted amount when relevant data changes
 useEffect(() => {
   if (exchangeRateData && exchangeRateData.rates && toCurrency) {
     const rate = exchangeRateData.rates[toCurrency.toLowerCase()]; // API rates are lowercase keys
     const numericAmount = parseFloat(amount);


     if (rate && !isNaN(numericAmount)) {
       setConvertedAmount(numericAmount * rate);
       setError(null); // Clear previous errors if conversion is successful
     } else {
       setConvertedAmount(null); // Reset if rate is not available or amount is invalid
       // Set error if trying to convert but no rate is found (and not self-conversion)
 ...
 const handleSwapCurrencies = () => {
   if (currencyOptions.flatMap(g => g.currencies).length < 2) return; // Need at least two distinct currencies to swap
   const temp = fromCurrency;
   setFromCurrency(toCurrency);
   setToCurrency(temp);
   // Rates will be re-fetched by the useEffect hook watching fromCurrency
 };
 ...
 const numericAmount = parseFloat(amount);
 const currentLocale = language === 'pt-PT' ? 'pt-PT' : 'en-US'; // Determine locale for formatting
  // Get flag country codes for display
...
return (
  <>
    ...
    <div className="bg-white p-4 sm:p-6 md:p-8 rounded-xl shadow-2xl w-full max-w-2xl mx-auto">
      <div className="space-y-6">
       ...
       <div className="grid grid-cols-1 gap-4 items-end">
         <AmountInput
           ...
           disabled={isLoadingCurrencies} // Disable if currencies are still loading
         />
       </div>


       {/* Currency Selectors and Swap Button Section */}
       <div className="flex flex-col sm:flex-row items-center gap-4">
         ...
         <CurrencySelector
           ...
           disabled={isLoadingCurrencies || currencyOptions.length === 0} // Disable if loading or no options
           className="w-full sm:w-auto flex-grow" // Allow selector to grow
         />
         {/* Swap Button */}
         <button
           ...
           className="p-3 mt-3 sm:mt-6 self-center sm:self-auto bg-slate-100 hover:bg-slate-200 rounded-full text-[#1865f2] transition-colors duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
           disabled={isLoadingCurrencies || isLoadingRates || currencyOptions.flatMap(g => g.currencies).length < 2} // Disable during loading or if not enough currencies
         >
       ...
       {/* Conversion Result Display Section */}
             ...
             {convertedAmount !== null ? (
               <>
                 ...
                 <div className="flex items-center justify-center text-xl font-medium text-slate-700">
                   {fromCountryFlagCode && <img src={`https://flagcdn.com/w20/${fromCountryFlagCode}.png`} alt={`${fromCurrency} flag`} className="w-5 h-auto mr-2 rounded-sm"/>}
                 ...
                 <div className="flex items-center justify-center text-2xl sm:text-3xl font-bold text-[#1865f2] my-2" aria-live="polite">
                   {toCountryFlagCode && <img src={`https://flagcdn.com/w20/${toCountryFlagCode}.png`} alt={`${toCurrency} flag`} className="w-5 h-auto mr-2 rounded-sm"/>}
                 ...
                 {exchangeRateData.rates[toCurrency.toLowerCase()] && (
                    <p className="text-sm text-slate-500">
                     1 {fromCurrency.toUpperCase()} = {getCurrencySymbol(toCurrency, currentLocale)} {formatCurrencyValue(exchangeRateData.rates[toCurrency.toLowerCase()], currentLocale, 2, 6)} {/* Show more precision for unit rate */}
                    </p>
                 )}
                ...
                // Display message if conversion is not possible (e.g., no rate, invalid input)
                fromCurrency !== toCurrency && !error && amount !== '' && parseFloat(amount) > 0 && (
                  <p className="text-slate-500">
                    {/* Differentiate messages based on the situation */}
     ...
     {/* Historical Chart Section - Display if not loading and currencies are selected */}
     {!isLoadingCurrencies && allCurrenciesMap && Object.keys(allCurrenciesMap).length > 0 && fromCurrency && toCurrency && fromCurrency !== toCurrency && (
       <div className="mt-10 pt-6 border-t border-slate-200">
             ...
             <button
               ...
               className={`px-4 py-2 text-sm font-medium rounded-md transition-colors
                 ${selectedTimeRange === preset.value
                   ? 'bg-[#1865f2] text-white shadow-md' // Enhanced style for selected
         ...
         <HistoricalChart
           ...
           allCurrenciesMap={allCurrenciesMap} // Pass the map for reference if needed by chart
           timeRangePresets={timeRangePresets} // Pass presets if chart needs to manage its own range changes
         />
       </div>
     )}
   </div>
